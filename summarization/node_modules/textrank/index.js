...
exports.TextRank = function (article, settings) {
    ...
    // Extracts the top `count` sentences
    this.extractSummary = function (count) {
...
function TextPreprocesser(article, userTokens, userTokensSplit) {
    const wordSeg = require('vntk').word_sent;
    
    // splitSentence takes a string {article} and turns it into an array of sentences
    // tokens are sentences, must end with (!?.) characters
    this.splitSentence = function (article) {
        return article.split(/[.!?]/g);
    }

    // given a sentence, split it up into the amount of words in the sentence
    this.segmentWord = function (sentence) {
        const noneWord = /[,:;'"\/\d]/g;
        // lowercase all the words in the sentences
        // and remove none word and trailing newline 
        const cleanedSentence = sentence.toLowerCase().replace(noneWord, "");

        // Split into the words thanks to word_sent
        return wordSeg.tag(cleanedSentence);
    }

    this.outputPreprocess = function (article) {
        let sentences = splitSentence(article);
